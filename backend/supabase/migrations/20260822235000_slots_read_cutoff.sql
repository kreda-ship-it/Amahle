-- ============================================================
-- 036 — availability reads each service's own cutoff
--
-- 035 gave services a latest_start_time and taught schedule_permits() to
-- honour it, but only when told which service it is being asked about.
-- get_visit_slots() was still calling employee_is_free() without that,
-- so every check fell back to the old rule and the new column did
-- nothing.
--
-- Three call sites, one argument each: the lead phase, the finish phase,
-- and the hand-over chunk. Identical to 034 otherwise.
-- ============================================================
